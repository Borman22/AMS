...
public class EroticaTime extends AbstractParticularFeatureChecker {
    ...
    public Map<String, Integer> checkFeature(PLKeeper plKeeper) {
        ...
        final TimeCode EROTICA_TIME_BEGIN = new TimeCode(properties.getProperty("EROTICA_TIME_BEGIN"));
        final TimeCode EROTICA_TIME_END = new TimeCode(properties.getProperty("EROTICA_TIME_END"));
        ...
        for (List<Event> program : plKeeper.getAllProgramsList()) {
            if (findProgramType(program.get(0), allProgramsDescriptions).matches(PROGRAM_TYPE)) {
                for (Event event : program) {
                    if (event.getTime() > EROTICA_TIME_END.getFrames() || event.getTime() + event.getDuration() > EROTICA_TIME_END.getFrames()) { // такое условие из-за того, что EROTICA_TIME_BEGIN == 0
