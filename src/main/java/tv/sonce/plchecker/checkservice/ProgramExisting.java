...
public class ProgramExisting extends AbstractParticularFeatureChecker {

    private final String PATH_TO_PROPERTY = "./properties/PLChecker/checkerservice/ProgramExisting.properties";
    ...
    public Map<String, Integer> checkFeature(PLKeeper plKeeper) {
        ...
        for (List<Event> currentProgram : plKeeper.getAllProgramsList()) {
            if (!isProgramExists(currentProgram.get(0), allProgramsDescriptions)) {
                for (Event event : currentProgram)
    ...
    private boolean isProgramExists(Event event, ProgramDescription[] allProgramsDescriptions) {
